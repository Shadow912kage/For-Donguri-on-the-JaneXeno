...
//  Version history
//    0.1.1: Changed 5ch TLD name, '.net' to '.io'.
//    0.1: Initial release

/* References
  どんぐりシステム
  https://donguri.5ch.io/cannon
  どんぐり大砲 API
  https://donguri.5ch.io/api
  https://donguri.5ch.io/confirm?url=<url encoded "thread URL">&date=<url encoded "res JPN style date time">
*/
...
var Exaxxion = {
  ...
  initialize: function () {
    ...
    var acornBase = "https://donguri.5ch.io";
    var commands = {
  ...
  getTargetUrl: function () {
    var urls = this.ThreadUrl.match(/https:\/\/(([-0-9A-Za-z]+)\.5ch\.io)\/([-0-9A-Za-z]+)\//);
    if (urls) {
  ...
  isAbleToFire: function (res) {
    var extendcmd = res.match(/<>( sssp:\/\/img\.5ch\.io\/ico\/[-\w!#\$%&'\(\)\*\+,\.:;=?]+? <br>)?\s+!extend:(.*?):(.*?):(\d+)?:(\d+)?(:donguri=(\d+)\/(\d))?:{0,2}\s+<br>/);
